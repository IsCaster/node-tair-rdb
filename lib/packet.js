...
exports.requestSmembersPacket = function (namespace, key) {
  var keyBuffer = transcoder.encode(key);  
  var ret = exports.writePacketBegin(consts.TAIR_REQ_SMEMBERS_PACKET, 8 + keyBuffer.length);
...
exports.requestSaddOrSremOrZremPacket = function(key, namespace, value, expire, version, op)
{
  var keyBuffer = transcoder.encode(key);
  var valueBuffer = transcoder.encode(value);
  if(op==1)
    var ret = exports.writePacketBegin(consts.TAIR_REQ_SADD_PACKET, 18 + keyBuffer.length + valueBuffer.length);
  else if(op=2)
    var ret = exports.writePacketBegin(consts.TAIR_REQ_SREM_PACKET, 18 + keyBuffer.length + valueBuffer.length);
  else if(op=3)
    var ret = exports.writePacketBegin(consts.TAIR_REQ_ZREM_PACKET, 18 + keyBuffer.length + valueBuffer.length);
  var pos = ret[1], packet = ret[0];
  //packet body begin
  packet[pos] = 0;
  pos++;
