...
exports.responseZrangebyscorePacket = function (data) {
  if (!data) {
    return {key: null, value: null};
  }
  var pos = 16;
  ...
  pos += 4;
  if (code !== 0) {
    console.log("responseZrangebyscorePacket return ,code="+code)
    return code;
  }
  ...
  pos += 4;
  if (vlen === 0) {
    console.log("responseZrangebyscorePacket return ,variable length="+vlen)
    return null;
  }
...
exports.requestSaddPacket = function(key, namespace, value, expire, version)
{
  var keyBuffer = transcoder.encode(key);
  var valueBuffer = transcoder.encode(value);
