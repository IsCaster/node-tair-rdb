...
{
  this.sadd_or_srem_or_zrem(key, namespace, value, callback, expire, version, 1) 
}
...
{
  this.sadd_or_srem_or_zrem(key, namespace, value, callback, expire, version, 2) 
}
...
{
  this.sadd_or_srem_or_zrem(key, namespace, value, callback, expire, version, 3) 
}
 ...
 * @params expire: seconds to expire, number, optional, default is 0 (not expired)
 * @params version: the version of data, using to solve concurrency conflicts, not commonly used in cache, optional, default is 0
 * @params op: 1 means sadd ,2 means srem, 3 means zrem
 */
Tair.prototype.sadd_or_srem_or_zrem=function (key, namespace, value, callback, expire, version, op) 
{
  var _expire = typeof expire === 'number' ? expire : 0;
  var _namespace = typeof namespace === 'number' ? namespace : 0;
  var _version = typeof version === 'number' ? version : 0;
  ...
  var _packet = packet.requestSaddOrSremOrZremPacket(key, _namespace, value, _expire, _version, op);
  var _addr = this.getDataNode(key);
  if (!_addr.success) {
    return callback(new Error('Tair: find datanode error'), false);
  }
