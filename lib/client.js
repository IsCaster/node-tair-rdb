...
Tair.prototype.smembers=function (key, namespace, callback, datatype)
{
  ...
  var _packet = packet.requestSmembersPacket(_namespace, key);
  ...
  comm.getData(_addr, _packet, function (err, data, len) {
    ...
    var d_list=[]
    if (datatype === 'string' && res && res.length>0) {
      res.forEach(function(e,i,a)
      {
        d_list.push(e.toString("utf8").replace('\u0000',''))
      })
      res=d_list
    }
    if (!res || res.length<=0) {
...
Tair.prototype.zrangebyscore=function (key, namespace, start, end, callback, datatype, limit, with_score )
{
  ...
  comm.getData(_addr, _packet, function (err, data, len) {
    ...
    var res = packet.responseZrangebyscorePacket(data);
    if(typeof res==='number')
    {
      if (!callback.called) {
        callback.called = true;
    ...
    else if(datatype)
    {
      res.forEach(function(e,i,a)
      {
        d_list.push(utils.decodeBuffer(e,datatype))
      })
      res=d_list
    }*/
...
Tair.prototype.zadd=function (key, namespace, value, score, callback, expire, version) 
{
  ...
  comm.getData(_addr, _packet, function (err, data, len) {
    ...
    if (returned.code !== 0) {
      if (!callback.called) {
        callback.called = true;
        return callback(new Error('Tair Error: Code ' + returned.code),false);
      }
    }
    if (!callback.called) {
      callback.called = true;
...
Tair.prototype.sadd=function (key, namespace, value, callback, expire, version) 
{
  ...
  if (!key || key.length === 0 || typeof key !== 'string') {
    return callback(new Error('Tair:sadd params - key lack.'), false);
  }
  if (!value || value.length === 0) {
    return callback(new Error('Tair:sadd params - value lack.'), false);
  }
  if (!this.inited) {
    //console.warn('Tair: client not init');
    this.requestQueue.push({method: 'sadd', args: Array.prototype.slice.apply(arguments), that: this});
    return;
  }
  var _packet = packet.requestSaddPacket(key, _namespace, value, _expire, _version);
  var _addr = this.getDataNode(key);
  if (!_addr.success) {
    return callback(new Error('Tair: find datanode error'), false);
  }
