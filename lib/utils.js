/*!
 * tair - lib/transcoder.js
 * Copyright(c) 2012 Taobao.com
 * Author: kate.sf <kate.sf@taobao.com>
 */

/**
 * utility module
 */

exports.encodeLong = function (number) {
  var rt = new Buffer(8);

  rt[7] = (number & 0xFF);
  rt[6] = ((number >> 8) & 0xFF);
  rt[5] = ((number >> 16) & 0xFF);
  rt[4] = ((number >> 24) & 0xFF);
  rt[3] = ((number >> 32) & 0xFF);
  rt[2] = ((number >> 40) & 0xFF);
  rt[1] = ((number >> 48) & 0xFF);
  rt[0] = ((number >> 56) & 0xFF);
  return rt;
};

exports.decodeLong = function (data) {
  var rv = 0;
  for (var i = 0; i < data.length; i++) {
    rv = (rv << 8) | ((data[i] < 0) ? (256 + data[i]) : data[i]);
  }
  return rv >>> 0;
};

exports.encodeInt = function (number) {
  var fg = new Buffer(4);
  fg.writeInt32LE(number, 0);
  return fg;
};

exports.decodeInt = function (data) {
  return data.readInt32LE(0);
}

exports.getInt = function (data, offset) {
  return data.readInt32LE(offset);
};

exports.encodeBoolean = function (b) {
  var rv = new Buffer(1);
  rv[0] = (b ? '1' : '0');
  return rv;
};

exports.encodeFloat = function (float) {
  var rf = new Buffer(8);
  rf.writeDoubleLE(float, 0);
  return rf;
};

exports.decodeBoolean = function (input) {
  return (input[0] == '1');
};

exports.longToIP = function (id) {
  var host = '';
  host += ((id & 0xff)).toString() + '.';
  host += (((id >> 8) & 0xff)).toString() + '.';
  host += (((id >> 16) & 0xff)).toString() + '.';
  host += (((id >> 24) & 0xff)).toString();

  var port = ((id >> 32) & 0xffff);
  return host;
};

function _decodeBuffer(buffer,datatype)
{
  if(buffer.constructor === Buffer)
  {
    if(datatype=='int')
    {
      return buffer.readInt32LE(0) 
    }
    else if(datatype=='float')
    {
      return buffer.readDoubleLE(0) 
    }
    else if(datatype=='double')
    {
      return buffer.readDoubleLE(0) 
    }
    else if(datatype=='long')
    {
      return exports.decodeLong(buffer)
    }
    else if(datatype=='string')
    {
      return buffer.toString("utf8").replace(/\u0000/g,'')
    }
    else if(datatype=='int8')
    {
      return buffer.readInt32LE(0)+buffer.readInt32LE(4)*4294967296
    }
    else if(datatype=='buffer')
    {
      return buffer
    }
  }
  else
  {
    return null  
  }
}

exports.decodeBuffer=function (buffer,datatype)
{
  if(!datatype || datatype=='buffer' || !buffer)
  {
    return buffer;
  }
  if(buffer.constructor === Array)
  {
    result=[]
    buffer.forEach(function(e,i,a){
        result.push(_decodeBuffer(e,datatype))
      })
    return result
  }
  else
  {
    return _decodeBuffer(buffer,datatype);
  }
}

